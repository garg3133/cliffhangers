...
with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
        ...
        for image_path in TEST_IMAGE_PATHS:
            ...
            Penalty = sum(weights)/2
            rating = ""
            if Penalty > 7.0:
                rating = "Very Poor"
            if Penalty > 5.0 and Penalty <= 7.0:
                rating = "Poor"
            if Penalty > 3.0 and Penalty <= 5.0:
                rating = "Fair"
            if Penalty > 1.0 and Penalty <= 3.0:
                rating = "Good"
            if Penalty <= 1.0:
                rating = "Excellent"
            
            meta_data = [image_path,total, Severity, Penalty, rating]
            print(meta_data)
print((time.process_time() - start)/8, "sec")    